...

{
  ...
  const tempConsequences: Record<number, { icon: string, desc: string }> = {
    0.0: { icon: '🎯', desc: 'Deterministic — always picks the highest probability token. Responses are consistent but can feel repetitive. Best for: factual Q&A, code.' },
    0.2: { icon: '📐', desc: 'Near-deterministic — very focused output with slight variation. Good for: precise answers, structured data extraction.' },
    0.5: { icon: '⚖️', desc: 'Balanced — mixes probability well. Answers feel natural and coherent without being unpredictable. This is what was used for your query.' },
    0.8: { icon: '🎨', desc: 'Creative — more variation and surprise. Good for: brainstorming, storytelling, open-ended questions. Risk: may drift from context.' },
    1.0: { icon: '🎲', desc: 'Maximum randomness — highly varied output. Can produce unexpected but interesting results. Not recommended for RAG — risks ignoring context.' }
  };
      ...
      >
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-cyan-300 font-bold text-lg">What does an LLM actually do? — The simple idea</h3>
          </div>
          <p className="text-slate-300 text-sm leading-relaxed">
            A Large Language Model is a next-word prediction machine at massive scale. Given the augmented prompt as input, it predicts the most likely next token, appends it, then predicts the next one — repeating this thousands of times until the answer is complete. The model does not &apos;understand&apos; your question the way a human does. It has learned statistical patterns across trillions of words of training text, and uses those patterns to produce output that is coherent, contextually aware, and — when RAG provides good context — factually grounded.
          </p>
        </motion.div>
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <h4 className="text-cyan-300 font-bold">① Two Types of Transformers — Encoder vs Decoder</h4>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
            ...
            <div className="bg-violet-500/5 border border-violet-500/40 p-4 rounded-xl flex flex-col">
              ...
              <ul className="text-xs text-slate-400 space-y-2 flex-grow">
                <li>→ Reads the entire input at once (bidirectional)</li>
                <li>→ Attends to ALL tokens simultaneously</li>
                <li>→ Produces one fixed vector per token</li>
                <li>→ Used for: understanding, embedding, classification</li>
                <li>→ Example: nvidia/nv-embedqa-e5-v5 (used earlier)</li>
              </ul>
              <div className="mt-4 inline-block bg-violet-500/20 text-violet-300 text-[10px] font-bold px-2 py-1 rounded-full w-fit">
                Used in Step 3 — Query Embedding
              </div>
            </div>
            ...
            <div className="bg-cyan-500/5 border border-cyan-500/40 p-4 rounded-xl flex flex-col">
              ...
              <ul className="text-xs text-slate-400 space-y-2 flex-grow">
                <li>→ Reads input left-to-right (causal/autoregressive)</li>
                <li>→ Can only attend to PAST tokens, not future ones</li>
                <li>→ Generates one NEW token at a time</li>
                <li>→ Used for: text generation, chat, completion</li>
                <li>→ Example: meta/llama-3.3-70b-instruct (used here)</li>
              </ul>
              <div className="mt-4 inline-block bg-cyan-500/20 text-cyan-300 text-[10px] font-bold px-2 py-1 rounded-full w-fit">
                Used in Step 7 — LLM Generation
              </div>
            </div>
          </div>

          <div className="bg-slate-800/50 border border-slate-700/40 rounded p-3 text-xs text-slate-300">
            <span className="mr-1">💡</span> RAG pipelines always use BOTH: an encoder to embed and retrieve, then a decoder to generate the final answer. They are two separate models serving two separate roles.
          </div>
        </motion.div>
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <h4 className="text-cyan-300 font-bold">② How the Answer Was Built — Token by Token</h4>
          </div>
          <p className="text-slate-300 text-sm mb-4">
            The model generates one token at a time in a loop. Each new token is appended to the output, and the model re-reads everything — input + generated tokens so far — before predicting the next one. The answer you saw was produced by hundreds of these individual prediction steps, each taking milliseconds.
          </p>
          ...
          <div className="bg-[#0f1117] border border-cyan-500/20 rounded p-3 text-xs text-slate-400">
            Each prediction step: model reads full context → computes probabilities over 128,000 vocabulary tokens → samples one token → appends it → repeats
          </div>
        </motion.div>
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <h4 className="text-cyan-300 font-bold">③ Temperature & Sampling — Controlling Randomness</h4>
          </div>
          <p className="text-slate-300 text-sm mb-4">
            The model does not pick the single most likely next token every time — that would produce repetitive, robotic output. Instead it samples from a probability distribution shaped by the temperature setting. This pipeline uses temperature=0.5.
          </p>
          ...
          <div className="bg-[#0f1117] border border-slate-800 rounded-xl p-5 mb-4">
            ...
            <div className="mt-6 border-t border-slate-800 pt-5">
              <div className="text-xs text-slate-400 mb-3 text-center">
                Temperature reshapes the probability distribution — lower = more peaked, higher = flatter
              </div>
...
<span className="text-slate-500">{"}"}</span>
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <h4 className="text-cyan-300 font-bold">⑤ Context vs Training — What the Model Actually Reads</h4>
          </div>
          <p className="text-slate-300 text-sm mb-4">
            LLMs have two sources of knowledge: their parametric memory (patterns baked in during training on trillions of tokens) and the in-context knowledge injected via the prompt. RAG deliberately uses both — the retrieved chunks provide specific, current facts; the model&apos;s training provides language understanding, reasoning, and synthesis ability.
          </p>
          ...
          <div className="flex flex-col items-center mb-6 mt-6">
             <div className="relative w-full max-w-[400px] h-[160px]">
                ...
                <div className="absolute top-[30px] left-0 right-0 flex justify-between items-center z-10 w-[380px] mx-auto">
                   ...
                   >
                      <span className="text-xs font-bold text-violet-300 mb-1 text-center leading-tight">📚 Parametric Memory</span>
                   ...
                   >
                      <span className="text-xs font-bold text-emerald-300 mb-1 text-center leading-tight">📄 In-Context Knowledge</span>
          ...
          <div className="flex flex-col gap-3">
             <div className="bg-[#0f1117] p-3 rounded-xl border-l-4 border-cyan-500 text-sm text-slate-300">
               ...
               Language patterns, reasoning, world knowledge, how to write coherent paragraphs — but possibly outdated or hallucinated specific facts.
             </div>
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <h4 className="text-cyan-300 font-bold">⑥ The Generated Answer — Fully Revealed</h4>
          </div>
          <p className="text-slate-300 text-sm mb-4">
            Below is the complete answer generated by LLaMA 3.3 70B for your query. Every factual claim in this answer traces back to the retrieved context chunks. Compare the answer to the source chunks — you will see the model synthesized and expanded them rather than copying them verbatim.
          </p>
             ...
             {retrievedChunks.map((chunk, i) => (
                <div key={i} className="border-l-4 border-cyan-500 bg-[#0a0c14] rounded-lg p-3 text-xs border border-r-slate-800 border-y-slate-800">
                   <div className="flex justify-between items-center mb-1">
                     <span className="font-bold text-slate-300">📄 Source used: <span className="text-emerald-400">{mode === 'web' ? chunk.source : `Chunk #${chunk.chunkIndex}`}</span></span>
                   ...
                   <div className="text-cyan-500 text-[10px] font-bold">
                     ↑ The LLM used facts from this chunk above
                   </div>
                </div>
             ))}
          </div>
        ...
        <motion.div variants={{ hidden: { opacity: 0 }, show: { opacity: 1 } }} className="mt-4 pt-6 border-t border-[#1e2235] flex flex-col items-center">
          <div className="flex items-center justify-center gap-2 flex-wrap text-[10px] sm:text-xs">
            ...
            <span className="text-cyan-500/50">→</span>
            <span className="bg-cyan-500/10 text-cyan-400 px-3 py-1.5 rounded-full font-mono border border-cyan-500/20 shadow-[0_0_10px_rgba(6,182,212,0.2)] text-center max-w-[120px]">Repeat × N</span>
            <span className="text-cyan-500/50">→</span>
            <span className="bg-cyan-500/10 text-cyan-400 px-3 py-1.5 rounded-full font-mono border border-cyan-500/20 shadow-[0_0_10px_rgba(6,182,212,0.3)]">Final Answer ✓</span>
          </div>
